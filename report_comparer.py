...
def checkLineDiff(before_file, after_file):
	...
	if before_length == after_length:
		all_matched = True
		for idx in range(5, before_length):
			# split the comma delimited string into a list
			before_entry = before_content[idx].split(',')
			after_entry = after_content[idx].split(',')
			
			if not(len(before_entry) == len(after_entry)):
				if before_entry[len(before_entry) - 1] == '\n':
					before_entry.pop()
				if after_entry[len(after_entry) - 1] == '\n':
					after_entry.pop()
			if (len(before_entry) > 0) and (len(after_entry) > 0):
				# compare each element in each line
				for indx in range(len(before_entry)):
					"""
					if (len(before_entry[indx]) > 0) and (len(after_entry[indx]) > 0):
						# remove \n character if necessary
						if before_entry[indx][-1] == '\n':
							before_entry[indx] = before_entry[indx][0:-2]
						if after_entry[indx][-1] == '\n':
							after_entry[indx] = after_entry[indx][0:-2]

						# remove quotes from each element if necessary
						if (before_entry[indx][0] == r'"') or (before_entry[indx][-1] == r'"'):
							before_entry[indx] = before_entry[indx][1:-2]
						if (after_entry[indx][0] == r'"') or (after_entry[indx][-1] == r'"'):
							after_entry[indx] = after_entry[indx][1:-2]

						print(before_entry[indx])
						print(after_entry[indx])
					"""
					
					if indx >= len(after_entry):
						print(indx)
						print(len(before_entry))
						print(len(after_entry))
						#print(before_entry[indx])
					if before_entry[indx] == after_entry[indx]:
						print('Match')
			"""
